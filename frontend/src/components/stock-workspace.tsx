...
export function StockWorkspace({ symbol }: StockWorkspaceProps) {
  ...
  useEffect(() => {
    ...
    if (useLlm) {
      nextState.debateReview.progress = {
        symbol,
        request_id: null,
        status: "running",
    ...
    const markModuleSuccess = <K extends ModuleKey>(key: K, value: WorkspaceState[K]["data"]) => {
      updateModule(key, (previous) => ({
        ...
        data: value,
        error: null,
        progress: key === "debateReview" ? null : previous.progress,
      }));
    };
    ...
    const startDebateProgressPolling = () => {
      ...
      const poll = async () => {
        try {
          ...
          updateModule("debateReview", (previous) => ({
            ...previous,
            progress,
          }));
        } catch {
    ...
    const loadModule = async <T, K extends ModuleKey>(
      key: K,
      loader: () => Promise<T>,
    ) => {
      try {
    ...
    void loadModule("debateReview", async () => {
      try {
        return await getDebateReview(symbol, {
          useLlm,
          requestId: debateRequestId,
        });
      } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="space-y-4">
          ...
          <div className="grid gap-3 lg:grid-cols-3">
            ...
            <Metric
              label="加载中模块"
              value={loadingModules.length === 0 ? "0" : loadingModules.map((item) => item.label).join("、")}
            />
            <Metric
              label="失败模块"
              value={failedModules.length === 0 ? "无" : failedModules.map((item) => item.label).join("、")}
            />
          </div>
      ...
      <SectionCard
        title="数据时效与落盘策略"
        description="这里明确说明当前页面的数据基准日、重新计算时机，以及哪些数据优先走本地。"
      >
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          <TextPanel
            title="本地优先的数据"
            content="股票基础资料、日线、公告、财务摘要都遵循“本地优先”。本地已有就直接读取；本地缺失或资料不完整时，再请求外部 provider，并把返回结果落到本地。"
          />
          <TextPanel
            title="实时计算的数据"
            content="Factor Snapshot、Review Report、Debate Review、Strategy Plan 都是在接口调用时基于当前本地数据即时计算的，不单独落盘为长期缓存。页面每次刷新都会重新计算。"
          />
          <TextPanel
            title="盘中数据说明"
            content="Trigger Snapshot 依赖盘中 provider。当前更偏向实时获取和轻量回退，不作为长期本地快照缓存；如果盘中 provider 不可用，会退回到日线 fallback 结果。"
          />
          <TextPanel
            title="当前股票基础资料状态"
            content={
              profile
                ? hasProfileGaps
                  ? `当前基础资料来源为 ${profile.source}，但仍有字段缺失，说明 provider 侧还没有返回完整资料。系统会优先尝试用本地缓存，必要时再自动补全。`
                  : `当前基础资料来源为 ${profile.source}，关键字段已经齐全。后续再次访问时会优先使用本地缓存。`
                : "等待基础资料模块返回后显示。"
            }
          />
        </div>
      ...
      <SectionCard title="股票基础信息" description="这里展示当前股票的基础资料和静态元信息。">
        {renderModuleState({
          ...
          content: profile ? (
            <div className="space-y-4">
              ...
              {hasProfileGaps ? (
                <StatusBlock
                  title="基础资料仍有缺口"
                  description="这通常意味着本地已有基础记录，但 provider 侧还没有补全完整字段。你可以稍后再次刷新，或先依赖其他模块继续分析。"
                  tone="error"
                />
              ) : null}
            </div>
      ...
      >
        {renderModuleState({
          ...
          loadingDescription: buildCalculationDescription(
            workspaceState.factorSnapshot.startedAt,
            clock,
            "当前会基于本地日线、公告和财务摘要即时计算。",
          ),
          ...
          content: workspaceState.factorSnapshot.data ? (
            <div className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
                ...
                <Metric label="重新计算" value="每次页面刷新都会重算" />
              </div>
              <div className="grid gap-4 lg:grid-cols-2">
                <SignalGroupList
                  title="偏强因子组"
                  groups={workspaceState.factorSnapshot.data.factor_group_scores
      ...
      >
        {renderModuleState({
          ...
          loadingTitle: "正在生成 Review Report",
          loadingDescription: buildCalculationDescription(
            workspaceState.reviewReport.startedAt,
            clock,
            "当前会基于本地数据重新组织技术、基本面、事件和情绪视角。",
          ),
            ...
            <div className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
                ...
                <Metric
                  ...
                  value={`${workspaceState.reviewReport.data.factor_profile.alpha_score} / ${workspaceState.reviewReport.data.factor_profile.trigger_score} / ${workspaceState.reviewReport.data.factor_profile.risk_score}`}
                />
                <Metric label="技术趋势" value={formatLabel(workspaceState.reviewReport.data.technical_view.trend_state)} />
                <Metric label="重新计算" value="每次页面刷新都会重算" />
              </div>
              <TextPanel title="最终结论" content={workspaceState.reviewReport.data.final_judgement.summary} />
              <div className="grid gap-4 lg:grid-cols-2">
                <TextPanel title="因子画像" content={workspaceState.reviewReport.data.factor_profile.concise_summary} />
                <TextPanel title="技术画像" content={workspaceState.reviewReport.data.technical_view.tactical_read} />
                <TextPanel
                  title="基本面画像"
                  content={
                    workspaceState.reviewReport.data.fundamental_view.quality_read ??
                    workspaceState.reviewReport.data.fundamental_view.data_completeness_note
                  }
                />
                <TextPanel
                  title="事件与情绪"
                  content={`${workspaceState.reviewReport.data.event_view.concise_summary} ${workspaceState.reviewReport.data.sentiment_view.concise_summary}`}
                />
              </div>
              <div className="grid gap-4 lg:grid-cols-2">
                <StringListPanel title="多头理由" items={workspaceState.reviewReport.data.bull_case.reasons} emptyText="当前没有多头理由。" />
                <StringListPanel title="空头理由" items={workspaceState.reviewReport.data.bear_case.reasons} emptyText="当前没有空头理由。" />
                <StringListPanel title="关键分歧" items={workspaceState.reviewReport.data.key_disagreements} emptyText="当前没有关键分歧。" />
                <StringListPanel title="最终关键点" items={workspaceState.reviewReport.data.final_judgement.key_points} emptyText="当前没有最终关键点。" />
              </div>
            </div>
          ) : null,
        })}
          ...
          content: workspaceState.debateReview.data ? (
            <div className="space-y-4">
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                <AnalystPanel
                  title="技术分析员"
                  summary={workspaceState.debateReview.data.analyst_views.technical.summary}
                  positivePoints={workspaceState.debateReview.data.analyst_views.technical.positive_points}
                  cautionPoints={workspaceState.debateReview.data.analyst_views.technical.caution_points}
                />
                <AnalystPanel
                  title="基本面分析员"
                  summary={workspaceState.debateReview.data.analyst_views.fundamental.summary}
                  positivePoints={workspaceState.debateReview.data.analyst_views.fundamental.positive_points}
                  cautionPoints={workspaceState.debateReview.data.analyst_views.fundamental.caution_points}
                />
                <AnalystPanel
                  title="事件分析员"
                  summary={workspaceState.debateReview.data.analyst_views.event.summary}
                  positivePoints={workspaceState.debateReview.data.analyst_views.event.positive_points}
                  cautionPoints={workspaceState.debateReview.data.analyst_views.event.caution_points}
                />
                <AnalystPanel
                  title="情绪分析员"
                  summary={workspaceState.debateReview.data.analyst_views.sentiment.summary}
                  positivePoints={workspaceState.debateReview.data.analyst_views.sentiment.positive_points}
                  cautionPoints={workspaceState.debateReview.data.analyst_views.sentiment.caution_points}
                />
              </div>
              <div className="grid gap-4 lg:grid-cols-2">
                ...
                <StringListPanel title="裁决关键点" items={workspaceState.debateReview.data.chief_judgement.decisive_points} emptyText="当前没有裁决关键点。" />
      ...
      <SectionCard title="Strategy Plan" description="展示结构化交易策略，只保留执行需要的关键字段。">
        {renderModuleState({
          ...
          loadingDescription: buildCalculationDescription(
            workspaceState.strategyPlan.startedAt,
            clock,
            "当前会根据最新本地数据和研究结论即时生成策略。",
          ),
          ...
          content: workspaceState.strategyPlan.data ? (
            <div className="space-y-4">
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                ...
                <StringListPanel title="避免条件" items={workspaceState.strategyPlan.data.avoid_if} emptyText="当前没有避免条件。" />
      ...
      >
        {renderModuleState({
          ...
          loadingDescription: buildCalculationDescription(
            workspaceState.triggerSnapshot.startedAt,
            clock,
            "当前会优先尝试盘中 provider，必要时自动退回日线 fallback。",
          ),
...
function renderModuleState({
  ...
  if (state.status === "error") {
    return (
      <StatusBlock
        title="加载失败"
        description={state.error ?? "发生未知错误，请稍后重试。"}
        tone="error"
      />
    );
  }
...
function LoadingPanel({
  ...
  const progressRatio =
    progress && progress.total_steps > 0
      ? Math.max(
          8,
          Math.min(100, Math.round((progress.completed_steps / progress.total_steps) * 100)),
        )
      : 18;
      ...
      {progress ? (
        <div className="space-y-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Metric
              label="步骤进度"
              value={progress.total_steps > 0 ? `${progress.completed_steps} / ${progress.total_steps}` : "等待后台返回"}
            />
          </div>
          <p className="text-sm leading-6 text-slate-700">{progress.message}</p>
          {progress.error_message ? (
            <StatusBlock title="后台提示" description={progress.error_message} tone="error" />
          ) : null}
          {progress.recent_steps.length > 0 ? (
            <div>
              <p className="text-sm font-semibold text-slate-900">最近步骤</p>
              <ul className="mt-2 space-y-2 text-sm leading-6 text-slate-700">
                {progress.recent_steps.map((item) => (
...
function AnalystPanel({
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <DebatePointList title="支持点" points={positivePoints} emptyText="当前没有支持点。" />
        <DebatePointList title="谨慎点" points={cautionPoints} emptyText="当前没有谨慎点。" />
      </div>
    </div>
  );
}
...
function buildLoadingDescription(startedAt: number | null, clock: number): string {
  return `请求已发出${formatWaitTime(startedAt, clock)}。模块会独立返回，你可以先查看其他已完成的内容。`;
}
...
function buildDebateLoadingDescription({
  ...
  if (!useLlm) {
    return `${waitText}当前使用规则版裁决，通常会在本地较快完成。`;
  }
  if (!progress) {
    return `${waitText}正在等待后台返回 LLM 裁决进度。`;
  }
