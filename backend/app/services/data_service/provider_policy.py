"""Provider capability policy definitions."""
...
_CAPABILITY_POLICIES: dict[MarketDataCapability, CapabilityPolicy] = {
    DAILY_BAR_CAPABILITY: CapabilityPolicy(
        ...
        notes="日线是长期主链输入，优先本地与局域网源；允许在远端失败时退回本地已落盘快照。",
    ),
    INTRADAY_BAR_CAPABILITY: CapabilityPolicy(
        ...
        notes="分钟线偏实时，不接受 stale fallback；当前以可用性优先。",
    ),
    ...
    FINANCIAL_SUMMARY_CAPABILITY: CapabilityPolicy(
        ...
        preferred_providers=("tushare", "baostock", "akshare"),
        allow_stale_fallback=False,
        require_local_persistence=True,
        notes="财务摘要优先使用本地快照，其次使用 Tushare 结构化主源；BaoStock 和 AKShare 仅作 fallback。",
    ),
    FINANCIAL_REPORT_INDEX_CAPABILITY: CapabilityPolicy(
        capability=FINANCIAL_REPORT_INDEX_CAPABILITY,
        preferred_providers=("cninfo",),
        allow_stale_fallback=False,
        require_local_persistence=True,
        notes="定期报告索引以正式披露源为准，本轮只作为真相源索引层。",
    ),
}
